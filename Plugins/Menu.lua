...
function ReceiveFriendMsg(CurrentQQ, data)
	if data.FromUin ==1962847213 then--防止自我复读
		  return 1 end
	if string.find(data.Content, "机器人菜单") == 1 then
		menu =  	"1.se图命令(不保证可用)有：\n来点/张/份/XX(可选)/(的)色/涩/瑟/黄/图(可以私聊)、漫画、插画、首页推荐、周排行、cos、私服、cos周排行、cos月排行、私服排行、炼点铜。(不可私聊)\n"..
					...
					"9.翻译功能: 翻译成中文(英语/日语): 内容 (一定要带英文冒号,标点符号之后的文字可能被截断)\n"..
...
function ReceiveGroupMsg(CurrentQQ, data)
	if data.FromUserId ==1962847213 then--防止自我复读
		  return 1 end
	menu = nil
	...
	if string.find(data.Content, "P站网址多少") == 1 then --问答插件 按需添加
	menu =  "官网:Pornhub.com"
	end
